...
public class Communication {
    ...
    private int[] getCoords(int maxlignes, int maxcolonnes, String message) {
        ...
        if (col >= maxcolonnes || lig >= maxlignes) {
            System.out.println("Les coordonnées que vous avez saisi ne sont pas valides. Veuillez essayer à nouveau.");
    ...
    public Deplacement demanderDeplacement(int maxlignes, int maxColonnes) {
          // On initialise par un tableau vide pour calmer le message d'erreur de IDE. (Possible NullPointerException)
          int[] coordDepart = new int[2], coordArrive = new int[2];
          coordDepart = this.getCoords(maxlignes, maxColonnes, "Veuillez saisir les coordonnées de la case de départ");
          coordArrive = this.getCoords(maxlignes, maxColonnes, "Veuillez saisir les coordonnés de la case destination");
